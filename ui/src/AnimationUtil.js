...
export const cinemagraphFilterExpression = (settings, duration, width, height, fps) => {
    ...
    switch (animationSettings.motionType) {
        ...
        case 'breathe':
            return `scale=iw*(1+(${motionIntensity}/100)*sin(2*PI*(t/${loopDuration}))):ih*(1+(${motionIntensity}/100)*sin(2*PI*(t/${loopDuration}))):eval=frame,` +
                    `pad=ceil(iw*(1+${motionIntensity}/100)):ceil(ih*(1+${motionIntensity}/100)):(ow-iw)/2:(oh-ih)/2,` +
